...
//setup margins
  ...
  setup_x_axis = function(graph){
    ...
    //setup domain to be mirror around 0
    var temp_xextent = d3.extent(options.data.map(function(d){ return d[options.xDomain]; }))
    var xextent = d3.max(temp_xextent.map(function(d){return Math.abs(d)})) + 1;
    scaleX.domain([-xextent,xextent]);
    graph.xextent = xextent; // need this for the lines later on
  ...
  setup_y_axis = function(graph){
    ...
    setup_lines = function(graph){
        svg = graph.svg;
        options = graph.options;
        page_options = graph.page_options;


        y_value =  options.neg_log_10_Q_value_threshold;
        scaleY = graph.scaleY;
        scaled_y_value = scaleY(y_value);

        svg.append("line")          // attach a line
        .style("stroke-dasharray", "3,3")  // colour the line
        .style("stroke", "grey")  // colour the line
        .attr("x1", 0)     // x position of the first end of the line
        .attr("y1", scaled_y_value)      // y position of the first end of the line
        .attr("x2", page_options.width)     // x position of the second end of the line
        .attr("y2", scaled_y_value);   


        x_value =  options.log_2_fold_change_threshold;
        scaleX = graph.scaleX;
        scaled_x_value = scaleX(x_value);

        svg.append("line")          // attach a line
        .style("stroke-dasharray", "3,3")  // colour the line
        .style("stroke", "grey")  // colour the line
        .attr("x1", scaled_x_value)     // x position of the first end of the line
        .attr("y1", page_options.height)      // y position of the first end of the line
        .attr("x2", scaled_x_value)     // x position of the second end of the line
        .attr("y2", 0);   
